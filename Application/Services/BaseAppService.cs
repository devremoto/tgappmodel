...
public partial class BaseAppService<T>(IBaseService<T> service, IUnitOfWork uow) : IBaseAppService<T>
    where T : class
{
    protected IBaseService<T> _baseService = service;
    protected IUnitOfWork _uow = uow;
    ...
    public void SaveJson<T>()
    {
        var dir = JsonFolder();

        var content = _baseService.GetAll().ToList().JsonSerialize();
